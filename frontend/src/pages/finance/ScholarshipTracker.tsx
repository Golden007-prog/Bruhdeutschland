  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border bg-card p-5 shadow-sm">
        ...
        {items.length === 0 && (
          <p className="mt-4 rounded-md border border-dashed bg-muted/30 p-6 text-center text-sm text-muted-foreground">No applications tracked yet.</p>
        )}
        {items.length > 0 && (
              ...
              return (
                <li key={s.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border bg-card p-3 text-sm">
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => remove(s.id)} aria-label={`Remove ${s.name}`} className="rounded p-1 text-muted-foreground hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
